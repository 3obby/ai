...
const CommunityPage = () => {
  ...
  const handleVote = async (ideaId: string, type: "upvote" | "downvote") => {
    try {
      ...
      const response = await fetch(`/api/vote/${ideaId}/vote`, {
        method: "POST",
        headers: {
      ...
      if (response.status === 403) {
        ...
        toast({
          ...
          description: `You need ${voteCost} tokens to ${type}vote.`,
        })
        return
      }
      ...
      setIdeas((currentIdeas) =>
        currentIdeas.map((idea) =>
          idea.id === ideaId ? { ...updatedIdea, hasVoted: type === "upvote" ? "up" : "down" } : idea
        )
      )
