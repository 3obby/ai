#!/usr/bin/env python3
"""
update_memory_1024.py - Updates Pinecone index with only changed files
This version uses 1024-dimensional embeddings compatible with MCP
"""
...
HASH_STORAGE_FILE = os.path.join(DIRECTORY_TO_INDEX, 'scripts', '.file_hashes_1024.json')
...
def get_embedding(text, openai_client):
    """Get embeddings from OpenAI (1024 dimensions)"""
    try:
...
def index_directory(directory_path=DIRECTORY_TO_INDEX, force_update=False):
    ...
    # Check dimension of the index
    stats = pinecone_index.describe_index_stats()
    dimension = stats.dimension
    if dimension != 1024:
        print(f"WARNING: Index dimension is {dimension}, but this script is configured for 1024-dimensional embeddings.")
        if not force_update:
            response = input("Continue anyway? (y/n): ")
            if response.lower() != 'y':
                print("Exiting due to dimension mismatch.")
                return
...
def main():
    ...
    print("Enhanced Memory Updater for Pinecone (1024-dimensional version)")
    print("-" * 60)
